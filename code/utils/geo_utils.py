...
# def get_align_mat(pred_Rs, gt_Rs, pred_ts, gt_ts):
#     '''
...
#     print(5)
#     Q = torch.sum(gt_Rs @ pred_Rs.transpose(1,2), dim=0).cpu() #sum over the n views of R_gt[i] @ R_pred[i].T
#     print(6)
#     print(Q.shape)
#     print(Q)
#     # a=torch.tensor([[-6.1176, -6.9607, -4.5562],[-5.7225, -1.0672,  9.3038],[-6.2397,  7.5897, -3.2121]], requires_grad=True)
#     # a=a.cuda()
#     # print(a)
#     # u,s,v = torch.svd(a)
#     # print(s)
#     Uq, _, Vqh = torch.svd(Q)
#     print(8)
#     sv = torch.ones(3)
#     sv[-1] = torch.det(Uq @ Vqh)
#     print(7)
#     R_opt = (Uq @ torch.diag(sv) @ Vqh).cuda()
#     print(3)
#     Rs_fixed = R_opt.reshape([1,3,3]) @ pred_Rs
...
#     constraints = []
#     obj = cp.Minimize(
#         cp.sum(cp.norm(gt_ts - (c_opt * pred_ts + torch.ones((n, 1), dtype=torch.double) @ t_opt), axis=1)))
#     # obj = cp.Minimize(cp.sum(cp.norm(gt_ts.numpy() - (c_opt * pred_ts.numpy() + t_opt_rep), axis=1)))
#     prob = cp.Problem(obj, constraints)
#     prob.solve()
#     print("status:", prob.status)
#     ts_fixed = c_opt.value * pred_ts + t_opt.value.reshape([1,3])

#     return Rs_fixed, ts_fixed
...
#     return R_opt.reshape([1,3,3]), c_opt.value, t_opt.value.reshape([1,3])
...
def align_rotations(pred_Rs, gt_Rs):
    '''
    :param pred_Rs: torch double - n x 3 x 3 predicted camera rotation
    :param gt_Rs: torch double - n x 3 x 3 camera ground truth rotation
    :param pred_ts: torch double - n x 3 predicted translation
    ...
    return R_opt#.reshape([1,3,3]).repeat(n, 0)
...
@torch.no_grad()
def prealign_cameras(device, R_pre, t_pre, t_gt):
...
# input M✖3
def scale_ts_coo(ts):
    scale = 1.0/torch.max(ts.max(), -ts.min())
    # print(scale)
    return ts*scale

# input M✖3
def scale_ts_norm(ts):
    cts = ts-ts.mean(dim=0, keepdim=True)
    scale = 1.0/torch.max(cts.norm(dim=1))
    # print(scale)
    return cts*scale
...
def norm_quats(quats):
    return quats/quats.norm(2, dim=1, keepdim=True)     
...
def batch_get_normalization_matrices(xs):
    """
    Given all the observed points return normalization matrices such that N[i] takes the
...
def normalize_M(M, Ns, valid_points=None):
    ...
    norm_M = (Ns @ norm_M).permute(0, 2, 1)[:,:,:2]  # [m,3,3]@[m,3,n] -> [m,3,n] -> [m,n,3] -> [m,n,2]
...
###############################################################
def addNoise(M, meanvalue, variance, percent):
    mask = torch.where(M!=0, 1, 0)                                     
    p_mask = torch.randint(0, 100, M.size())
    p_mask = torch.where(p_mask<percent*100, 1, 0)                    
    noise = torch.normal(meanvalue, variance, M.size())*mask*p_mask    
    newM = M+noise
    return newM
