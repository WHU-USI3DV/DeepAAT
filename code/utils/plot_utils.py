...
def plot_cameras(Rs_pred, ts_pred, pts3D, Rs_gt, ts_gt, Rs_error, ts_error, conf, phase, scan=None, epoch=None):
    data = []
    data.append(get_3D_quiver_trace(ts_gt, Rs_gt[:, :3, 2], color='#86CE00', name='cam_gt', cam_size=2))
    data.append(get_3D_quiver_trace(ts_pred, Rs_pred[:, :3, 2], color='#C4451C', name='cam_learn', cam_size=2))
    # data.append(get_3D_scater_trace(ts_gt.T, color='#86CE00', name='cam_gt', size=2))
    # data.append(get_3D_scater_trace(ts_pred.T, color='#C4451C', name='cam_learn', size=2))
    data.append(get_3D_scater_trace(pts3D, '#3366CC', '3D points', size=0.5))

    fig = go.Figure(data=data)
    fig.update_layout(title='Cameras: Rotation Mean = {:.5f}, Translation Mean = {:.5f}, Points num = {}'.format(Rs_error.mean(), ts_error.mean(), pts3D.shape[1]), showlegend=True)
